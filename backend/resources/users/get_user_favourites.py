...
def main(event, context):
    ...
    experience_ids = list(user.get('favourite_experiences', set()))  # Convertimos el set a lista
    ...
    for experience_id in experience_ids:
        try:
            ...
            if 'Items' in response and len(response['Items']) > 0:
                favourites.append(response['Items'][0]) 
        except Exception as e:
            return build_response(500, {'error': f'Failed to get user favourites experience: {str(e)}'})

    return build_response(200, {'favourite_experiences': list(favourites)})